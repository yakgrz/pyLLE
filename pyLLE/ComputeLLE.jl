...
#for now just set decay of Braggs and loop reflection to be pretty sharp approximately dirac delts
σ_Bragg = zeros(length(μ),1) .+ .0001
...
CoupleLoopShift=ifft_plan* (Couple_Loop_Mirror)#.*length(μ)
...
function SSFM½step(A0, B0, it, param)
    ...
    function CPLf(vv)
       # retcplf .= 1im .*zeros(length(μ),1)
        ...
        #i think we avoid tR in the loop mirror term since kext and alpha are already in terms of FSR
